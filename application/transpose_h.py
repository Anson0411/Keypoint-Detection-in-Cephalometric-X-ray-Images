...
class TransPoseH(nn.Module):
    ...
    def __init__(self, cfg, **kwargs):
        ...
        d_model = cfg['MODEL']['DIM_MODEL']  # embedding : 想用多長的 vector 來表示一張圖
        dim_feedforward = cfg['MODEL']['DIM_FEEDFORWARD']
        encoder_layers_num = cfg['MODEL']['ENCODER_LAYERS']
        n_head = cfg['MODEL']['N_HEAD']
        pos_embedding_type = cfg['MODEL']['POS_EMBEDDING']  # positional encoding 的型態('none', 'learnable', 'sine')
        w, h = cfg['MODEL']['IMAGE_SIZE']

        self.reduce = nn.Conv2d(pre_stage_channels[0], d_model, 1, bias=False)
        self._make_position_embedding(w, h, d_model, pos_embedding_type)  # positional encoding
